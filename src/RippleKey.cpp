...
RippleKey RippleKey::make_RippleKey(ripple::KeyType const& keyType,
    std::string rawseed)
{
    auto seed = ripple::parseGenericSeed(rawseed);
...
{
    if (keyType && rawseed)
        return RippleKey::make_RippleKey(*keyType, *rawseed);
    if (rawseed)
